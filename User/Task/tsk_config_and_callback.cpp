...
uint32_t Pre_Uart1_Flag = 0, Uart1_Flag = 0;
...
void Uart1_Alive_Check(uint8_t *flag)//1为正常通信
{
    static uint8_t mod20 = 0;//50zh检查一次
	mod20++;
    if(mod20 == 20)
    {
        mod20 = 0;
        if(Uart1_Flag == Pre_Uart1_Flag)
            *flag = 0;
        else
            *flag = 1;
